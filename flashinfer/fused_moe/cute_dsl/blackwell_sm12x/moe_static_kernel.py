            ...
            while is_valid_tile:
                ...
                for output_tile_idx in range(0, output_tile_cnt, 1, unroll=4):  # type: ignore[call-overload]
                    ...
                    for epi_m in cutlass.range_constexpr(epi_rest_m):
                        ...
                        # Original: pair_idx iterates (row, col) in flat order,
                        #   each lane handles the same col for all rows → different
                        #   tok per lane → uncoalesced global writes.
                        # New: outer loop over rows, inner 32 lanes each handle
                        #   lane_id-th pair_col of this row → 32 lanes write 64
                        #   consecutive bytes to the same token → fully coalesced.
                        #   tok/wv loaded once per row by every lane (no shuffle).
                        warp_epi_rows = (
                        ...
                        while local_row < warp_epi_rows:
                            ...
                            # Every lane reads the same tok/wv directly — no shuffle.
                            tok = _ld_shared_i32(
                            ...
                            # 32 lanes × 2 bf16 = 64 consecutive bytes in output,
                            # all targeting the same token row → coalesced.
                            global_col = (
                                tile_n_base_cur
                                + warp_n_base
                                + lane_id * Int32(2)
